    ...
    private static boolean solution(String line) {
        ...
        for (int i = 0; i < lengthA + 1; i++) {
            for (int j = 0; j < lengthB + 1; j++) {
                if ((i - 1) >= 0 && reslut[i - 1][j] && array[0].charAt(i - 1) == array[2].charAt(i - 1 + j)) { // dp[i-1][j]=true表示s3[1, i-1+j]由s1[1,i-1]和s2[1,j]交错组成,如果s1.charAt(i-1)与s3.charAt(i-1+j)相等，那么很显然dp[i][j]也为true
                    reslut[i][j] = true;
                }
                if ((j - 1) >= 0 && reslut[i][j - 1] && array[1].charAt(j - 1) == array[2].charAt(i + j - 1)) { // dp[i][j-1]=true表示s3[1, i+j-1]由s1[1,i]和s2[1,j-1]交错组成,如果s2.charAt(j-1)与s3.charAt(i+j-1)相等，那么很显然dp[i][j]也为true
                    reslut[i][j] = true;
